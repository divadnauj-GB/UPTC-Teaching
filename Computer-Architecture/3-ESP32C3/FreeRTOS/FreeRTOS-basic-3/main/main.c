...
{
    synchro_t * sync = (synchro_t *) pvParameters;
...
void vTask2(void *pvParameters)
{
    ...
    while (1)
    {
        if (sync->flag == 1) {
            if (sync->repeat > 0) {
                uint8_t red[3] = {0, 100, 0}, black[3] = {0, 0, 0};
...
void app_main(void)
{
    ...
    synchro_t * sync = pvPortMalloc(sizeof(synchro_t));
    sync->flag = 0;
    sync->repeat = 10;
    xTaskCreate(vTask1, "Task 1", 2048, sync, 1, NULL);
    xTaskCreate(vTask2, "Task 2", 2048, sync, 1, NULL);
