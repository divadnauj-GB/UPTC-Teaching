...
find ./ -type f -name '*.vhd'  -exec \
  ghdl -i --std=08 --workdir=build --ieee=standard --work=trafic_lights {} \;

# GHDL analyze
$GHDL -m --work=trafic_lights --workdir=build --std=08 trafic_lights_tb
...
# GHDL run
runcmd="$GHDL -r --work=trafic_lights --workdir=build --std=08 trafic_lights_tb \
  --max-stack-alloc=0 \
  --vcd=waves.vcd \
